...
USER_AGENT = 'PatriotAdminBot/1.0 (+https://ctf.local)'


def get_admin_cookie():
    s = requests.Session()
    s.post(f'{BASE_URL}/api/login', json={'username': ADMIN_USER, 'password': ADMIN_PASS}, verify=False)
    for c in s.cookies:
        if c.name == 'session':
            return c.value
    return None
...
def run_bot(conversion):
    ...
    options.add_argument("--disable-features=BlockThirdPartyCookies,ThirdPartyStoragePartitioning")
